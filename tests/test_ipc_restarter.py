...
from tests import LOG, Block, assert_logs
from tests.fsfix import mk, attach_teardown, teardown
...
def test_defaults(): # dot junkie
    ...
    def assert_(foo):
        try:
            ...
            teardown() # have to do this manually because of nose issue 202
...
def test_filesystem():
    ...
    def _test(file_to_change, hack=False):
        mk( ('foo.py', '"bar"')
          , ('foo.conf', 'bar')
    ...
    def test(file_to_change, hack=False):
        try:
            _test(file_to_change, hack=False)
        finally:
            teardown() # have to do this manually because of nose issue 202
...
def test_signal():
    ...
    def _test(generation, signum, expected):
    ...
    def test(generation, signum, expected):
        try:
            _test(generation, signum, expected)
        finally:
            teardown() # have to do this manually because of nose issue 202
