        ...
        try:
            ...
            if value is NOT_INITIALIZED:
                ...
                if opened is not None:
                    value = self.build(key, name, opened, entry)
                    #print 'new build: ',str(key)
                    is_new = True
                    self._pack(entry, value)
                    self.commit()
                #else: print 'old build: ',str(key)
...
#========= end of the N.L.'s code ==========    


class CacherClass( type ):
    """ This metaclass is responsible to do file and module caching"""
    def __call__( cls,  *a, **kw ):
        """ One needs only one storage for caching for all instances \
